...
func _bridge_oblique() -> void:
	...
	sensor_mat.set_shader_parameter("vignette", 0.0)
	sensor_mat.set_shader_parameter("dither", false)
	var target: Vector3 = Vector3(364.0, 8.0, 55.0)           # GG landing, southern portion (deck end z-8 <-> coast z~145)
	cam.near = 5.0
	cam.far = 3000.0
	cam.fov = 54.0
	cam.position = Vector3(364.0, 245.0, 430.0)              # DUE SOUTH, elevated, looking N: coast near/bottom, deck+model far/top
