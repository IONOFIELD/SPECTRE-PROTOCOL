...
func _bridge_oblique() -> void:
	...
	sensor_mat.set_shader_parameter("vignette", 0.0)
	sensor_mat.set_shader_parameter("dither", false)
	var target: Vector3 = Vector3(300.0, 5.0, 200.0)          # the GG landfall / Presidio
	cam.near = 5.0
	cam.far = 3000.0
	cam.fov = 52.0
	cam.position = Vector3(119.0, 316.0, 375.0)               # matches the user's HDG 046 / EL 51 view (N upper-left)
