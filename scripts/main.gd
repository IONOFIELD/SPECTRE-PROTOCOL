...
func _bridge_oblique() -> void:
	...
	sensor_mat.set_shader_parameter("vignette", 0.0)
	sensor_mat.set_shader_parameter("dither", false)
	var target: Vector3 = Vector3(364.0, 8.0, 90.0)           # the GG SF landing (deck now reaches z=160)
	cam.near = 5.0
	cam.far = 3000.0
	cam.fov = 54.0
	cam.position = Vector3(150.0, 250.0, 420.0)              # SW + elevated, looking NE: bridge from Marin down onto the SF shore
